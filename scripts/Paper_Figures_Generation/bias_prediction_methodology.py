...
from src.non_hierarchical.plotting_functions import plot_latent_data_1d
...
labels = [
    "$\phi_Y \sim \mathcal{GP}(m=1,k_{RBF}(s,s'|v=1,l=3))$",
    "$\phi_B \sim \mathcal{GP}(m=-1,k_{RBF}(s,s'|v=1,l=10))$",
    "$\phi_Z = \phi_Y+\phi_B$",
]
fig.legend(
    labels, fontsize=legend_fontsize, bbox_to_anchor=(0.83, 0.85), ncols=1, loc=10
)
plt.tight_layout()
plt.show()
fig.savefig(f"{out_path}fig03.pdf", dpi=300, bbox_inches="tight")
...
plt.tight_layout()
plt.show()
fig.savefig(f"{out_path}fig04.pdf", dpi=300, bbox_inches="tight")
