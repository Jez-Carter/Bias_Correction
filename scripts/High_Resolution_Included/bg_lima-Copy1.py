...
df_sample = pd.read_csv('/data/notebooks/jupyterlab-biascorrlab/data/AP_Daily_Snowfall_044_Sample.csv')
sample_data = df_sample['prsn'].to_numpy()
sample_data = sample_data.reshape(len(np.unique(month_values)),sample_size,min_days) # ordered [months,sites,days]
sample_data = np.moveaxis(sample_data, -1, 0) # adjusting the axes so that it's [days,months,sites]

obs_data = np.load(f'/data/notebooks/jupyterlab-biascorrlab/data/AP_Daily_Snowfall_011_sample.npy') #[days,months,sites]
# climate_data = np.load(f'/data/notebooks/jupyterlab-biascorrlab/data/AP_Daily_Snowfall_044.npy') #[days,months,sites]
...
#Converting to JAX array (needed for Numpyro)
jobs_data = jnp.array(obs_data[:,[0],:]) # JAX doesn't like object arrays so data for each month needs to be the same shape
# jclimate_data = jnp.array(climate_data[:,[0],:])
...
np.save(f'/data/notebooks/jupyterlab-biascorrlab/data/AP_BGLima_MCMC_Samples_Obs.npy',samples_array)
